...
public class King extends Piece{
	...
	/** Extra variables for King class to keep a track of king's position. */
	private int x,y;
	...
	public ArrayList<Cell> move(Cell state[][],int x,int y)
	{
		...
		for(int i=0;i<8;i++) {
			if(posx[i]>=0&&posx[i]<8&&posy[i]>=0&&posy[i]<8 && (state[posx[i]][posy[i]].getpiece()==null||state[posx[i]][posy[i]].getpiece().getcolor()!=getcolor())) {
	...
	public boolean isindanger(Cell state[][])
    {
    	...
    	for(int i=x+1;i<8;i++)
    	{
    		if(state[i][y].getpiece()==null) {
				continue;
			} else if(state[i][y].getpiece().getcolor()==getcolor()) {
				break;
			} else if (state[i][y].getpiece() instanceof Rook || state[i][y].getpiece() instanceof Queen) {
				return true;
			} else {
				break;
			}
    	...
    	for(int i=y+1;i<8;i++)
    	{
    		if(state[x][i].getpiece()==null) {
				continue;
			} else if(state[x][i].getpiece().getcolor()==getcolor()) {
				break;
			} else if (state[x][i].getpiece() instanceof Rook || state[x][i].getpiece() instanceof Queen) {
				return true;
			} else {
				break;
			}
		...
		while(tempx<8&&tempy>=0)
		{
			...
			else if(state[tempx][tempy].getpiece().getcolor()==getcolor()) {
				break;
			} else if (state[tempx][tempy].getpiece() instanceof Bishop || state[tempx][tempy].getpiece() instanceof Queen) {
				return true;
			} else {
				break;
			}
		...
		for(int i=0;i<8;i++) {
			if(posx[i]>=0&&posx[i]<8&&posy[i]>=0&&posy[i]<8 && state[posx[i]][posy[i]].getpiece()!=null && state[posx[i]][posy[i]].getpiece().getcolor()!=getcolor() && state[posx[i]][posy[i]].getpiece() instanceof Knight) {
				return true;
			}
		}
		...
		{
			for(int i=0;i<8;i++) {
				if(pox[i]>=0&&pox[i]<8&&poy[i]>=0&&poy[i]<8 && state[pox[i]][poy[i]].getpiece()!=null && state[pox[i]][poy[i]].getpiece().getcolor()!=getcolor() && state[pox[i]][poy[i]].getpiece() instanceof King) {
					return true;
				}
			}
		}
