...
public class Pawn extends Piece{
	...
	public ArrayList<Cell> move(Cell state[][],int x,int y)
	{
		...
		if(getcolor()==0)
		{
			...
			if(y>0&&state[x-1][y-1].getpiece()!=null&&state[x-1][y-1].getpiece().getcolor()!=getcolor()) {
				possiblemoves.add(state[x-1][y-1]);
			}
			if(y<7&&state[x-1][y+1].getpiece()!=null&&state[x-1][y+1].getpiece().getcolor()!=getcolor()) {
				possiblemoves.add(state[x-1][y+1]);
			}
		}
		else
