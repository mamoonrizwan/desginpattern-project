...
public class King extends Piece {
	...
	public King(String i, Color c) {
		setId(i);
		setPath(c.equals(Color.WHITE) ? "White_King.png" : "Black_King.png");
		setColor(c);
	...
	public boolean isInDanger(Cell state[][]) {
		...
		if (getcolor().equals(Color.WHITE)) {
			if (x > 0 && y > 0 && state[x - 1][y - 1].getpiece() != null
					&& state[x - 1][y - 1].getpiece().getcolor().equals(Color.BLACK)
			...
			if (x < 7 && y > 0 && state[x + 1][y - 1].getpiece() != null
					&& state[x + 1][y - 1].getpiece().getcolor().equals(Color.WHITE)
					&& (state[x + 1][y - 1].getpiece() instanceof Pawn))
				return true;
